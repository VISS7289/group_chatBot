...
from myChat import  data_processing_cornell_movie_dialogs as dp
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer, 
    ...
    print_loss = 0
    if loadFilename:
        start_iteration = model['iteration'] + 1
...
def InitVoc(myVoc):
    voc=mc.Voc("")
    voc.name=myVoc['name']
    voc.trimmed=myVoc['trimmed']
    voc.word2index=myVoc['word2index']
    voc.word2count=myVoc['word2count']
    voc.index2word=myVoc['index2word']
    voc.num_words=myVoc['num_words']
    return voc

def InitEmbedding(voc):
    embedding = nn.Embedding(voc.num_words,mp.hidden_size)
    return embedding

def InitEncoder(embedding):
    encoder=EncoderRNN(mp.hidden_size,embedding,mp.encoder_n_layers,mp.dropout)
    return encoder

def InitDecoder(embedding,voc):
    decoder = LuongAttnDecoderRNN(mp.attn_model, embedding, mp.hidden_size, voc.num_words,
                    mp.decoder_n_layers, mp.dropout)
    return decoder

def InitEncoderOptimizer(encoder):
    encoder_optimizer = optim.Adam(encoder.parameters(), lr=mp.learning_rate)
    return encoder_optimizer

def InitDecoderOptimizer(decoder):
    decoder_optimizer=optim.Adam(decoder.parameters(), lr=mp.learning_rate * mp.decoder_learning_ratio)
    return decoder_optimizer

model=torch.load(mp.PATH)
myVoc=model['voc_dict']
voc=InitVoc(myVoc)
embedding=InitEmbedding(voc)
embedding.load_state_dict(model['embedding'],strict=True)
encoder = InitEncoder(embedding)
encoder.load_state_dict(model['en'],strict=True)
decoder=InitDecoder(embedding,voc)
decoder.load_state_dict(model['de'],strict=True)
encoder = encoder.to(device)
decoder = decoder.to(device)
encoder_optimizer=InitEncoderOptimizer(encoder)
encoder_optimizer.load_state_dict(model['en_opt'])
decoder_optimizer=InitDecoderOptimizer(decoder)
decoder_optimizer.load_state_dict(model['de_opt'])
encoder.eval()
decoder.eval()

searcher = GreedySearchDecoder(encoder, decoder)

_,pairs = dp.dataProcessing()
dialogueNum = len(pairs)
