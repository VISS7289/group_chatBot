...
datafile = os.path.join(corpus, "datadeal.txt")
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        if iteration % print_every == 0:
            ...
            batchlistw.append(print_loss_avg)
            print_loss = 0
    ...
    filedraw = open('sampleList.txt', 'w')
    for ip in batchlistw:
        filedraw.write(str(ip))
        filedraw.write('\n')
    filedraw.close()
...
loadFilename = os.path.join(directory, '{}_{}.tar'.format(checkpoint_iter, 'checkpoint'))
...
if loadFilename:
    # 如果训练和加载是一条机器，那么直接加载
    loadFilename = 'myChat/' + loadFilename
